...
Feature: Credit card payment
  I want to test the credit card payment feature

  @CC_Scenario1
  Scenario: Credit card payment with one item
    Given user is on home page and test data present in "/CreditCardPaymentFeature/Scenario01.json"
  ...
  Scenario: exception occurred while place order
    Given user is on home page and test data present in "/CreditCardPaymentFeature/Scenario02.json"
    When login using the credentials
    And add item to cart
    And checkout the cart
    And select billing address
